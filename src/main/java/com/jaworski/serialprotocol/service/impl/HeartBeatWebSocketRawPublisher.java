...
public class HeartBeatWebSocketRawPublisher implements WebSocketPublisher {

    private static final Logger LOG = LogManager.getLogger(HeartBeatWebSocketRawPublisher.class);
    ...
    public void publishForAllClients(String message) {
        wsSessionManager.getWebSocketSessions().stream()
                ...
                .filter(webSocketSession -> webSocketSession.getUri().toString().contains(SessionType.HEARTBEAT.getName()))
                .forEach(session -> {
                    try {
